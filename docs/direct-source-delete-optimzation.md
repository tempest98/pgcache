# Skip Invalidation for Direct Source DELETE on Multi-Table Queries
...
**Current behavior**: DELETE on orders (Direct source, multi-table JOIN, no LIMIT) → cache MISS (invalidation + re-population).

**Desired behavior**: DELETE on orders → cache HIT (row removed in-place, serve-time re-evaluates).

## Root Cause

In `row_uncached_invalidation_check` (`src/cache/writer/cdc.rs:443`), for Direct source DELETE on a multi-table query:
...
**LEFT JOIN optional side (NOT EXISTS decorrelation)**: DELETE could expand result (shrinks exclusion set). But these tables get `OuterJoinOptional` or `OuterJoinTerminal` source, NOT `Direct`. The optimization doesn't affect them.
...
Source assignment verified in `query_table_update_queries` (`src/query/update.rs:44-55`): outer join optional-side tables are reclassified from `Direct` to `OuterJoinOptional`/`OuterJoinTerminal`. Only INNER JOIN tables retain `Direct`.

## Change

### `src/cache/writer/cdc.rs` — `row_uncached_invalidation_check`

Add early return for Direct source DELETE (non-LIMIT):
...
5. Manually verify no other integration tests assert `cache_miss` for Direct source DELETE on multi-table non-LIMIT queries (search for patterns)
